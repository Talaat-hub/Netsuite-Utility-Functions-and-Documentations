...
## YOU NEED TO APPLY ROLES FOR THE SCRIPT TO RUN MANUALLY FROM NETSUITE, BUT THERE IS A WAY TO PERFORM IT INSIDE THE OBJECT FILE

13 - Inside the scriptdeployment tag of the object.xml file after we set the record type we resume and say:
...
```
    <scriptdeployments>
        <scriptdeployment scriptid="customdeploy_sdf_test">
            ...
            <recordtype>[customrecord_az_emp_mahmoud]</recordtype>
            <status>RELEASED</status>
            <allroles>T</allroles> // Apply all roles
            <allemployees>T</allemployees> // Apply all employee
        </scriptdeployment>
    </scriptdeployments>
```
